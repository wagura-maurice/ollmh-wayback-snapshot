...
CREATE TABLE wp_upcoming_projects (
  ...
  CONSTRAINT fk_upproj_page    FOREIGN KEY (page_id)         REFERENCES wp_pages (id)        ON DELETE CASCADE  ON UPDATE CASCADE,
  CONSTRAINT fk_upproj_dept    FOREIGN KEY (department_id)   REFERENCES wp_departments (id)  ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT fk_upproj_related FOREIGN KEY (related_page_id) REFERENCES wp_pages (id)        ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT fk_upproj_cover   FOREIGN KEY (cover_media_id)  REFERENCES wp_media_assets (id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Construction phases / milestones (the tuition block has phase 1 complete)
CREATE TABLE wp_upcoming_project_phases (
  id           BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT fk_upphase_project FOREIGN KEY (project_id) REFERENCES wp_upcoming_projects (id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Progress-gallery images per project from the shared media library
CREATE TABLE wp_upcoming_project_media (
  id         BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  project_id BIGINT UNSIGNED NOT NULL,
  ...
  CONSTRAINT fk_upm_project FOREIGN KEY (project_id) REFERENCES wp_upcoming_projects (id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_upm_media   FOREIGN KEY (media_id)   REFERENCES wp_media_assets (id)      ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Optional donation pledges toward an upcoming project's funding goal
CREATE TABLE wp_upcoming_project_pledges (
  id           BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  project_id   BIGINT UNSIGNED NOT NULL,
  ...
  CONSTRAINT fk_uppledge_project FOREIGN KEY (project_id) REFERENCES wp_upcoming_projects (id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```

**Relationships**
- `wp_upcoming_projects.page_id` → `wp_pages.id` binds each project to the `/upcoming-projects.html` page.
- `wp_upcoming_projects.department_id` → `wp_departments.id` links a project to its owning department (e.g., Diagnostic Centre, Nursing School).
- `wp_upcoming_projects.related_page_id` → `wp_pages.id` models the inline "SEE DIAGNOSTIC CENTRE" cross-reference to a detail page.
- `wp_upcoming_projects.cover_media_id` and `wp_upcoming_project_media.media_id` → `wp_media_assets.id` reuse the shared media library.
- `wp_upcoming_project_phases.project_id` → `wp_upcoming_projects.id` tracks phased delivery (e.g., tuition block phase 1 complete).
- `wp_upcoming_project_pledges.project_id` → `wp_upcoming_projects.id` records donations toward funding goals.
